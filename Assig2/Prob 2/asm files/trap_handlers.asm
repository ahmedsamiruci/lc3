...
HALT_TRAP               ; store the current process status
                    ...
                    STR R7, R6, #1      ; Store the thread PC [R7] into the PCBx[1] to restore execution later
                        ; PCBx[2] is holding the MPU value and shouldn't be changed
                    STR R1, R6, #3      ; Store R1 into PCBx[3]
                    STR R2, R6, #4      ; Store R2 into PCBx[4]
                    TRAP    xFF
...
increment_process
            ADD R1, R1, #6      ; R1 <- the next thread address (cur thrd + 6)
...
RESUME_PROC
            ...
            ; PCBx[2] is holding the MPU value and shouldn't be changed
            LDR R1, R0, #3      ; R1 <- PCBx[3]
            LDR R2, R0, #4      ; R2 <- PCBx[4]
            ...
            STI R5, MR_SM_ADDR  ; Mem[ MR_SM_ADDR ] = 0
                ...
                ; Set the MPU register
            LDR R5, R0, #2      ; R5 <- PCBx[2] = MPU
            STI R5, MR_MPU_ADDR ; Mem[ MR_MPU_ADDR ] = MPU value saved in PCBx[2]
